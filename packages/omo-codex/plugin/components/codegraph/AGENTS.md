...
Serve pipeline (`src/serve.ts`): OMO SOT config gate (`[codex].codegraph.enabled=false` → unavailable stub) → workspace safety gate (`auto_init="safe"` requires an owned Git root for new indexes; indexes over `max_index_db_bytes`, default 2 GiB, are blocked) → resolve binary (`OMO_CODEGRAPH_BIN` env → bundled → provisioned `~/.omo/codegraph/bin` → PATH) → Node support gate (major 20-24; >=25 crashes CodeGraph mid-indexing; `CODEGRAPH_ALLOW_UNSAFE_NODE=1` overrides) → auto-provision into `~/.omo/codegraph` unless `codegraph.auto_provision=false` → bridge JSON-RPC to `codegraph serve --mcp`. Project cwd resolved from `OMO_CODEGRAPH_PROJECT_CWD` → `OMO_CODEGRAPH_SESSION_START_CWD` → `PWD` → wrapper cwd. When the binary is missing, disabled, or blocked by workspace policy, `mcp-unavailable.ts` still answers `initialize`/`tools/*` with the skip reason so Codex startup never fails.

Hook pipeline (`src/hook.ts` + `src/session-start-worker.ts`): `hook session-start` applies the same workspace safety gate before probing `status --json` (2s timeout); uninitialized project → spawns a detached `hook session-start-worker` (repeat safety gate → prepare workspace + gitignore → `status --json` with a 5s timeout → `init` or `sync` with a 60s timeout) and emits a SessionStart `additionalContext` notice. Worker outcomes append to `~/.omo/codegraph/session-start.jsonl`. `hook post-tool-use` emits init guidance when a codegraph tool result indicates an uninitialized project. `cli.js` with no hook subcommand falls through to serve.
...
| `[codex].codegraph` config keys (`enabled`, `auto_init`, `auto_provision`, `max_index_db_bytes`, `watch_debounce_ms`, `excluded_roots`, trusted `install_dir`) | `packages/omo-codex/plugin/shared/src/config-loader.ts` |
