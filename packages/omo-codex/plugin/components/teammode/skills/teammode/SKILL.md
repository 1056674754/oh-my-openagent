---
name: teammode
description: "Codex-only team orchestration with durable, script-managed state. MUST USE when the user asks to create, run, coordinate, inspect, archive, or delete a team of agents, threads, or sessions, or to work as a team in parallel. First inspect available tools and choose native MultiAgentV2 agents, Codex App threads as fallback, or plain subagents when neither team transport exists. The main session leads; each member owns a concrete part or perspective. Use a team when parallel work needs coordination; use plain subagents for isolated work. Triggers include team mode, teammode, make or run a team, coordinate threads, and parallel Codex threads."
---
